...
with open(args.char_file) as f:
    ...
    inverse_vocab_initializer = tf.lookup.KeyValueTensorInitializer(values, keys, value_dtype=tf.string, key_dtype=tf.int32)
...
def inference(tensors):
    byte_array = tf.concat([["<s>"], tf.strings.unicode_split(tf.strings.regex_replace(tensors, " +", " "), "UTF-8"), ["</s>"]], axis=0)
